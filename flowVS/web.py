...
from graph import generate_digraph_from_action_list, build_action_list, AffectTaskAffect, CreateActionAffect
from resware_model import build_models, Task
from settings import ACTION_LIST_DEF_ID, WEB_TOKEN, DOT_EXE_PATH

app = flask.Flask(__name__)
parser = argparse.ArgumentParser()
parser.add_argument('-id', type=int, nargs='?', help='ID of the Action List to render')
args = parser.parse_args()
ActionListID = args.id or ACTION_LIST_DEF_ID
print('Rendering ActionListID: ' + str(ActionListID))

@app.route('/')
def stream():
	if request.headers.get('Authorization', '') != WEB_TOKEN:
		abort(401)
	alist = build_action_list(build_models(), ActionListID)
	digraph = '\n'.join(generate_digraph_from_action_list(alist))
	run = subprocess.run([DOT_EXE_PATH, '-Tsvg'], stdout=subprocess.PIPE, input=bytes(digraph, 'utf-8'))
...
def api_action_list():
	...
	groups = build_action_list(build_models(), ActionListID).groups
